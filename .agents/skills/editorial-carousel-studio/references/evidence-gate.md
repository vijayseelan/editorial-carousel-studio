...
During production, copy the approved register into `content/claim-register.json`. Bind each slide to its claims through a `claimIds` array in `content/story.json`; a claim ID written only in visible footer copy is not a valid binding. The renderer must validate the actual register rather than merely confirming that a validation policy exists.
